...
export function ChallengeView() {
  ...
  useEffect(() => {
    if (originalDataset && selectedFeatures.length > 0) {
      ...
      const filteredDataset: Dataset = {
        train: originalDataset.train.map(filterFeatures),
        test: originalDataset.test.map(filterFeatures),
        featureNames: selectedFeatures.map(i => originalDataset.featureNames[i]),
        labelName: originalDataset.labelName,
        classes: originalDataset.classes
      };

      setDataset(filteredDataset);
    }
  }, [selectedFeatures, originalDataset]);
  ...
  if (!region || !dataset || !originalDataset) {
    return (
          ...
          <div className="bg-gradient-to-r from-blue-500 to-cyan-500 p-6">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="text-center flex-1">
                ...
                <p className="text-blue-100 text-sm mt-1">{region.daimyo}</p>
              </div>
              <div className="w-32" />
            </div>
                ...
                return (
                  <div key={step.id} className="flex items-center">
                    ...
                    {index < steps.length - 1 && (
                      <ChevronRight className="w-4 h-4 md:w-5 md:h-5 text-white/50 mx-1" />
                    )}
                  </div>
                );
              })}
            ...
            <div className="mb-6 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-xl p-4 md:p-6 border-2 border-yellow-200">
              ...
              <div className="flex items-center space-x-4 text-sm">
                ...
                {regionProgress?.best_accuracy && regionProgress.best_accuracy > 0 && (
                  <div className="flex items-center space-x-2">
                    <span className="text-gray-600">自己ベスト:</span>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between">
                  ...
                  <button
                    ...
                    className="flex items-center space-x-2 bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 text-white px-8 py-3 rounded-lg font-bold shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    <span>次へ：モデルを選ぶ</span>
                    <ChevronRight className="w-5 h-5" />
                  </button>
              ...
              <div className="space-y-6">
                ...
                <div className="flex justify-between">
                  ...
                  <button
                    onClick={handleTrain}
                    disabled={isTraining}
                    className="flex items-center space-x-2 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white px-8 py-3 rounded-lg font-bold shadow-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
